...
# App title
st.set_page_config(page_title=" UTY COOPERATION")

col1, col2 = st.columns([0.5, 3])
col2.markdown("#### PHEONIX - AMAZON LISTING SUPPORT & CHAT 💬")
col1.image("./logo/phoenix.png", width=80)

hf_email = st.secrets['EMAIL']
hf_pass = st.secrets['PASS']

st.sidebar.image("./logo/UTY_logo.png", width=200)

st.sidebar.markdown("""
                    ___Please follow the format if you want to make the listing___
                    
                    > Product: <name of product>, Key: <important list of keys>

                    ___Example:___

                    > Product: Coconut Milk Tea, Key: Vietnamese, fresh, no added sugar

                    """)

def create_prompt(input_user:str) -> str:
    product_name = input_user.lower().split("key")[0]
    keys = input_user.lower().split("key")[1]
    prompt=f"""
        You are doing the role of a Amazon Product Listing expert. 
                Giving a {product_name} with some important key {keys}, 
                write a whole product for me to sale in Amazon US. 
